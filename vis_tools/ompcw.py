...
class Profiler:
      ...
      for member in self.enslist:
        ...
        for thread in self.threadlist:
          ...
          for case in self.caselist:
            ...
            print(pinfo)
            self.stats_list[core][member][thread].append(atime)
      ...
      while(nl < num_lines):
        if(lines[nl].find('Parallel Timing Statistics') > 0):
         ...
         #  print('Start Parallel Timing Statistics')
          nl, atime = self.parallel_time_stats(lines, nl)
          nl += num_lines
        nl += 1
  ...
  def parallel_time_stats(self, lines, nl):
    ...
    while(going):
     ...
     #  atime = float(nlist[2])
     # #print('      Total time:', atime)

      if(name == 'oops::GETKFSolver::computeWeights'):
        atime = float(nlist[2])
       #print('      Total time:', atime)
  ...
  def plot_bar(self, ax, core, member):
    title = '%d MPItasks, %d members computeWeights time' %(core, member)
