...
class Profiler:
  ...
  def __init__(self, debug=0, enslist=None,
    ...
    self.color_list = ['red', 'orange', 'blue', 'lightblue', 'cyan', 'darkgreen', 'darkblue']

  def process(self):
    nc = 0
    self.filelist = []
    for n in self.enslist:
      flnm = '%s/omp1/n%dm%do1/log.obs.np%d_nens%d' %(self.workdir, self.ncore, n, self.ncore, n)
  ...
  def plot_percent(self):
   ...
   #plt.yscale("log", base=10)
    ...
    imgname = 'percent_%dc_obs.png' %(self.ncore)
  ...
  def find_name2idx(self, name):
   #print('find idx for  name = ' + name)
    ...
    idx = -1
    for n in range(nl):
      if(name == self.stats_list[0][2][n]['name']):
        idx = n
        break
  ...
  def plot_functions(self):
   ...
   #plt.title(title)
    ...
    if(self.linear):
      imgname = 'lin_%dc_obs.png' %(self.ncore)
    else:
      imgname = 'log_%dc_obs.png' %(self.ncore)
