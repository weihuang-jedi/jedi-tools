...
class Profiler:
  ...
  def __init__(self, debug=0, threadlist=[], corelist=[], enslist=[],
    ...
    self.color_list = ['cyan', 'blue', 'red', 'darkgreen', 'darkblue']
    self.function_list = ['applyWeights', 'computeWeights', 'total']
    self.fullfunction_list = ['oops::GETKFSolver::applyWeights', 'oops::GETKFSolver::computeWeights',
                              'util::Timers::Total']
   ...
   #plt.ylabel('Time (sencond)')
    ...
    for nc in range(len(self.corelist)):
      ...
      for nm in range(len(self.enslist)):
        ...
        if(nt > 1):
          self.plot_act(axes[nm, nc], core, member)
  ...
  def plot_act(self, ax, core, member):
...
if __name__== '__main__':
 ...
 #workdir = '/work/noaa/gsienkf/weihuang/jedi/intelcase0'
  enslist = [10, 20, 40, 80]
 #enslist = [10, 20, 40]
  threadlist = [1, 2, 4, 8]
  linear = 1
