...
class Profiler:
  ...
  def __init__(self, debug=0, linear=0, output=0, corelist=None, filelist=None):
    ...
    self.functionlist = ['Total', 'measured', 'ObsSpace', 'computeHofX', 'State',
                         'measurementUpdate', 'ObsVector', 'write', 'read',
                         'mask(ObsVector)', 'packEigen', 'ones', '~ObsSpace', 'ObsErrors']
  ...
  def find_func_name_idx(self, cn, name):
   #print('len(self.stats_list) = ', len(self.stats_list))
   #print('cn = ', cn)
   #print('name = ', name)
   #print('self.stats_list[cn] = ', self.stats_list[cn])
   #print('self.stats_list[cn][2] = ', self.stats_list[cn][2])
    nl = len(self.stats_list[cn][2])
    indx = 0
    for n in range(nl):
      if(name == self.stats_list[cn][2][n]['name']):
  ...
  def plot_select_funcs_bar(self, varname='avg'):
    ...
    title = 'solver %s time Bar Plot for selected Functions' %(varname)
   ...
   #the actual graph:
    ...
    imgname = 'solver_select_functions_%s_CPUtime_bar_plot.png' %(varname)
...
if __name__== '__main__':
  ...
  mtpn = 1
 #nodelist = [40, 50, 60, 70, 80, 90, 100, 120, 140]
  nodelist = [70, 80, 90, 100, 120, 140, 160]
  corelist = []
  ...
  filelist = []
  for n in range(len(corelist)):
    flnm = '%s/soca_solver.%dt%dn_%dp/stdoutNerr/stdout.00000000' %(rundir, mtpn, nodelist[n], corelist[n])
