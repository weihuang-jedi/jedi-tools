...
class Profiler:
  ...
  def __init__(self, debug=0, linear=0, output=0, corelist=None, filelist=None):
    ...
    self.functionlist = ['Total', 'computeHofX', 'process', 'apply', 'save',
                         'GetValues', 'ObsSpace', 'LocalInterpolator', 'State',
                         'deserialize', 'Geometry', 'postFilter', 'preProcess']
...
if __name__== '__main__':
 ...
 #nodelist = [1, 2, 4, 8, 16]
  ...
  rundir = '/work2/noaa/gsienkf/weihuang/ufs/soca/new-soca-observer'
  filelist = []
  for n in range(len(corelist)):
    flnm = '%s/soca_observer.%dt%dn_%dp/stdoutNerr/stdout.00000000' %(rundir, mtpn, nodelist[n], corelist[n])
