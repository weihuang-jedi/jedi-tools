...
class Profiler:
  """ Constructor """
  def __init__(self, debug=0, linear=0, corelist=None, filelist=None):
  ...
  def plot_par_bar(self, pv):
    ...
    title = '%d Parallel Functions 20 members' %(pv)
    ...
    lgname = [str(self.corelist[0]) + 'm20', str(self.corelist[1]) + 'm10',
              str(self.corelist[2]) + 'm20', str(self.corelist[3]) + 'm10']
   ...
   #the actual graph:
    ...
    ax.bar(idx-width, ts[0], width, color='r')
    ax.bar(idx, ts[1], width, color='b')
    ax.bar(idx+width, ts[2], width, color='orange')
    ax.bar(idx+2.0*width, ts[3], width, color='cyan')
...
if __name__== '__main__':
  debug = 1
  workdir = '.'
  corelist = [24, 24, 240, 240]
  filelist = ['n24m20/log.run.np24_nens20', 'n24m10/log.run.np24_nens10',
              'n240m20/log.run.np240_nens20', 'n240m10/log.run.np240_nens10']
  linear = 0
