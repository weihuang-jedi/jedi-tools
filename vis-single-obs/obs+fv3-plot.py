...
#------------------------------------------------------------------------------
 ...
 #datadir = '%s/analysis.getkf.80members.120procs/increment/' %(casedir)
  casedir = '/work2/noaa/gsienkf/weihuang/jedi/singleobs/%s' %(casename)
  ...
  for ntile in range(1,7,1):
   ...
   #datafile = '%s20200110.030000.fv_core.res.tile%s.nc' %(datadir, ntile)
    datafile = '%s20151206.120000.fv_core.res.tile%s.nc' %(datadir, ntile)
  ...
  if(addobs):
    if(casename == 'surf'):
     #filename = '%s/ioda_v2_data/ioda_v2_sfc_ps_obs_2020011006.nc4' %(casedir)
      filename = '%s/ioda_v2_data/ioda_v2_psurf.nc4' %(casedir)
    else:
